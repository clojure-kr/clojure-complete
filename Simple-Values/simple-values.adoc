...
식별자 앞에 콜론을 두 개 붙이면 해당 이름공간을 가진 키워드로 확장된다. 예를 들어, 현재의 이름공간이
``user``일 때 다음을 실행하면, ``user`` 이름공간이 붙은 키워드로 확장되는 것을 볼 수 있다.
...
아닌 문자로 시작해야 하고, 그 뒤에 `*`, `+`, `-`, `=`, `?`, `!`, `$`, `%`, `&`, `_`, `|`, `<`, `>` 같은 기호와 문자들이 올 수
있다.
