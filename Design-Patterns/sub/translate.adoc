...
페드로: 이 문제로 구글을 검색해 보니, 저장 버튼을 구현하는 가장 일반적인 접근법은 페펜토 패턴이네요. originator와 caretaker, memento 색체가 필요해요.
...
페즈로: originator는 저장하기 원하는 객체 또는 상태에요. 예를 들면 텍스트 박스 안의 텍스트이지요. caretaker는 상태를 저장하는 일을 맡아요. 그리고 memento는 상태를 보관하는 객체이고요.
