...
페드로: 그럴 수도 있지만, 예를 들어, Ui에서 서로 결함된 수백개의 부품들(components)이 있는 경우에는 이 중개자 패턴이 구세주 역할을 할 수 있어요.
...
독립 보안 위원회가 해커 다티 헤블(Dartee Hebl)이 자신의 계좌에 십억 달러의 잔고를 가지고 있는 것을 포착했다. 그래서 연관 계좌들과의 거래 내역을 추적해야 한다.
****
...
페드로: 관잘자들을 추가하면 되요. 잔고의 변화가 큰 경우에만, 이것을 통지하고 그 이유를 추적하면 되죠. 먼저 `Observer` 인터페이스를 다음과 같이 정의합니다.
...
페즈로: `Tracker` 클래스에서 이 관찰자 객체들을 관리하는 일을 맡습니다.
...
이브: 왜 관찰자를 따로 두 개를 만들었나요? 다음처럼 한 개만 만들어도 될 것 같은데.
...
페드로: 일을 한 개만 처리한다는 윈칙(Single responsibility principle)을 따른 거죠.
...
이브: 그래요, 사실 관찰자는 함수를 등록하는 한 가지 방법일 뿐이 거든요.  그리고 나서 다른 함수가 그 등록된 함수를 호출하게 되죠.
...
이브: 우선 `add-money` 함수가 더 깔끔해졌어요. 단순히 돈을 더하는 일만 하고 있죠. 아울러 watcher는 ``add-money`` 함수에서의 변경 내용 뿐만아니라,  user에게 일어나는 모든 상태 변화를 추적할 수 있어요.

Pedro: Explain please.
페드로: 좀 더 설명해 주세요.

Eve: If there is provided another secred method secret-add-money for changing balance, watchers will handle that as well.
이브: 만약 또다른 함수 `secret-add-money`가 잔고를 바꾼다 하더라도, watcher는 그것도 처리할 수 있다는 겁니다.
...
버티 프레이시(Bertie Prayc)가 우리 서버에서 중요한 데이터를 훔쳐 BitTorrent 시스템을 통해 공유하고 있다.  버티의 가짜 계정을 만들어 그의 평판을 떨어뜨려야 한다.
****

Pedro: BitTorrent system based on .torrent files. We need to write Bencode encoder.
페드로: BitTorrent 시스템은 .torrent 파일에 기반하고 있어서 Bencode 인코더를 만들어야 해요.

Eve: Yes, but first let's agree on the format spec
이브: 그렇군요, 하지만, Bencode 포맷에 대해 먼저 알야아 겠네요.
...
* 컨데이너도 두 개가 지원된다.
** 값의 리스트는 ``l<contents>e``로 인코딩된다. (예) [1, "Bye"] = li1e3:Byee
** 값의 맵은 ``d<contents>e``로 인코딩된다. (예) {"R" 2, "D" 2} = d1:Ri2e1:Di2ee
*** 키는 단순히 문자열이고, 값은 모든 Bencode 요소가 허용된다.

Pedro: Seems easy.
페드로: 쉬워 보이네요.

Eve: Maybe, but take into account that values may be nested, list inside list, etc.
이브: 그럴 수도 있지만, 값들은 중첩될 수 있다는 점을 고려해야 해요. 예를 들면 리스트 안에 리스트 같은.

Pedro: Sure. I think we can use Interpreter pattern for bencode encoding.
페드로: 물론이죠. bencode 인코딩에는 통역자 패턴을 사용할 수 있을 것 같네요.
...
페드로: 마지막으로 bencode 인코딩 문자열을 만들 수 있어요ㅣ.
[source,java]
----
// discredit user
...
페드로: 기능이 충실하려면 코드량은 많아질 수 밖에 없지요.
...
이브: 맞아요, 통역자 패턴은 단순히 트리를 처리하는 함수들일 뿐인 거죠.
...
한 법무 법인의 시스템 관리자 크리스토펴 매튼 & 파트(Cristopher, Matton & Pharts)는 보고 시스템이 많은 메모리를 소모해서 가비지 컬렉터가 끊임없이 시스템을 몇 초 동안 멈추게 한다는 것을 발견했다. 그것을 고쳐야 한다.
****
...
페드로: 많은 점 데이터를 사용하는 실시간 차트 프로그램 때문이예요. 메모리를 많이 소모하거든요. 그래서 가비지 컬렉터가 시스템을 멈추게 하는 거고요.
...
페드로: 이 패턴의 경우에는 두 가지가 필요해요. 프로그램 실행 시점에 대부분의 점 데이터를 미리 계산하는 것과, 캐시 객체를 만들기 위해 생성자 대신에 정적인 팩토리 메소드를 이용하는 것이 그것이죠.
...
이브; 아니에요, 훨씬 더 유연해요. 세 개의 점이나 정수가 아닌 값을 캐시해야 할 필요가 있을 때에는 이차원 배열을 사용할 수 없으니까요.
...
포드래 베스퍼는 시합에서 속임수를 사용한 우리를 잡았다. 우리에겐 하나의 선택권이 있다: 경찰에 체포되거나 그의 슈퍼 기사를 시합에 참여하도록 돕는 것이다.
...
페드로: 비슷하지만, 똑같지는 않아요. 코만도는 군인이어서 시합에 참여가 허용되지 않죠. 그래서 우리가 맞추어 넣은 거구요. 하지만 기사는 시합 참여가 가능하니, 맞출 필요가 없구요, 기존 것에 기능을 추가해야 해요.
...
페드로: 우선 점수를 치기 위해 접근하는 인터페이스 대신에 클래스 ``Knight``를 만들어요. 그러고 나서 2개의 생성자를 만드는데, 하나는 표준 행위를 위한 디폴트이고, 데코레이트된 객체에 호출을 위임하는 데코레이트된 생성자이죠.

Eve: Is it fair to use abstract class instead of interface?
이브: 인터페이스 대산 추상 클래스를 쓰는게 맞아요?
...
페드로: 아니요, 하지만 비슷한 행위를 하는 2개의 클래스를 피하고, 각 데코레이트된 객체를 디본 구현으로 채우는 거죠. 각 메소드를 구현을 강체하는 대신.
...
페드로: FBI 요구 사항을 충족하는 2개의 데코레이터, 그리고 슈퍼 기사를 만들 수 있어요, Galahad같은 행위를 가진, 슈퍼 갑옷을 입고, 점수도 50점 더 높고.
...
이브: 네, 단지 파뭐 갑옷 데코레이터에만 주의를 하는 거죠.
...
데렌 바트는 칵테일 만드는 시스템을 관리하고 있다. 이 시스템은 불편했는데, 칵테일을 만든 후 바트는 직접 손으로 쉐이커에서 남은 원료를 빼내야 했기 때문이다. 이것을 자동화하라.

Pedro: Can we get access to his codebase?
페드로: 바트의 코드베이스에 접근할 수 있나요?

Eve: No, but he sent some APIs.
이브: 아니요, 하지만 바트가 API를 몇 개 보냈줬어요.
...
페드로: 바트는 소스에 손 대는 원치 않아요, 대신에 ``IBar`` 인터페이스의 구현한 클래스를 추가로 몇 개 만들어서 남은 재료를 자동으로 제거할 필요가 있어요.
...
페드로: 프록시 패턴을 구현하죠, 얼마전에 그에 대해 읽었어요.
...
페드로: 네, 게다가 덤으로 우리가 기존 기능을 깨지 않아도 되죠.
...
페드로: 우리가 한 것은 단지 기능을 기존에 이미 테스트된 ``StandardBar``에 위임한 거에요.

Eve: But you also substracts used ingredients from BarDatabase
이브: 하지만 여전히 ``BarDatabase``에서 쓰다 남은 원료를 제거하고 있어요.

Pedro: We assume, they are decoupled.
페드로: 우리가 가정했는데요, 그것들은 분리되어 있어요.
...
이브: 글세요, 모르겠네요. 제가 아는 바로는 함수 합성을 사용한다는 거죠.
...
이브: 함수의 집합을 단일 객체으로 그룹화하기 위해 프로토콜과 타입을 이용하죠.
...
이브: 하지만 구현 관점에서 보면 이런 패턴들은 모두 같아요, 어떤 것을 랩핑하고 호출을 랩퍼에게 위임하죠. "랩퍼"는 이런 패턴들에 맞는 이름이에요.
...
Hurece's Sour Man의 채용 대행사 여직원들이 구인 요청에 맞는 후보들를 확인하고 있다. 문제는 직무는 고객들이 만드는데, 자격요건은 채용 대행사에서 만든다는 것이다. 그들이 협업을 할 수 있는 유연한 방법을 제공하라.
...
페드로: 모든 특정 자격 요건은 이 인터페이스를 구현한 것이에요.
...
페드로: 생각해 보면, 이 자격 요건의 계층도가 인사부에서 설계된다는 거죠.

Eve: Ok.
이브: 그렇군요.

Pedro: And they have a Job hierarchy, where each specific job is subclass as well.
페드로: 그리고 ``Job`` 계층도가 있고요, 또한 특정 직무(job)들은 그 서브 클래스들이죠.
...
페드로: 이 시스템은 객체보다는 클래스가 더 인기가 있을 때 설계었죠, 그래서 여기껏 그대로죠.
...
페드로: 네, 잘 들어요. 자격 요건이 있는 직무는 완전히 분리된 계층도이고, 그것은 고객에 의해 개발돼요. 브릿지 패턴으로 이 두 계층도를 분리하고, 독자적으로 유지되도록 하죠.
...
페드로: ``Job``, ``CongnitectClojureDeveloperJob``, ``OracleJavaDeveloperJob``가 또 다른 계층도죠.
...
페드로: 맞아요! 이것이 인사부가 이 시스템을 이용해서 후보자를 찾는 방법이에요.
...
페드로: 여기에 포인트가 있어요. 고객은 ``Job`` 은 추상으로, ``JobRequirement``는 구현으로 사용하죠. 고객들은 단지 직무(job)을 설명을 달아서 만들고, 인사부는 그 설명을 특정 JobRequirement의 집합으로 바꾸죠.
...
이브: 오케이, 알겠어요, 하지만 이 문제는 인위적이에요. 기본적으로 우리가 필요한 것은 구상과 구체 사이의 협업 방법입니다.

Pedro: Maybe, but I want to see your clojure way solution for that specific problem using bridge pattern.
페드로: 아마도, 하지만 브릿피 패턴으로 이 특수한 문제를 클로저로 어떻게 푸는지 알고 싶어요.
...
이브: 인사과는 개발자들 같아요, 이 추상의 구현을 제공합니다.
...
이브: 후에, 새 job이 만들어지면, 하지만 자격요건이 개발되어 있지 않고 그러한 job 타입을 위한 구현도 없으면, adhoc 계층도를 사용해서 원상복구해요.
...
페드로: 오 그리고 인사부는 분기값을 위한 구현을 제공하지 않는다면 ::senior-java, 디스패치 값을 지닌 메소드는 호출 될까?
