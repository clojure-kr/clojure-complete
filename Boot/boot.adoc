...
많은 유연함과 확장성으로 인해, 현재는 Boot가 Leiningen에 비해 어리지만 시간이 지날수록
Leiningen 사용자가 Boot로 많이 넘어 올 것으로 예상된다.
...
우리가 잘 알고 있는 클로저 함수들은 imutable 자료형을 다룬다. 마찬가지로 Boot는 한 개의
작업을 수행할 때, 입력 파일들에 대해 어떤 작업을 수행한 후, 그 결과를 그 입력 파일들에
다시 덮어쓰지 않는다. 대신에, 그 전의 입력 파일들(fileset)은 임시 디렉토리(~/.boot/tmp)
아래에 먼저 저장해 놓는다. 그리고 그 입력 파일들에 작업을 수행한 결과 파일들을 target
디렉토리(디폴트는 target)에 저장한다. 이러한 과정이 여러 개의 작업에 대해 반복적으로
수행된다.


== Boot의 설치

먼저 다음의 과정을 따라 Boot를 설치해 보자. (설치 시점: 2015-12-24)

.Windows
* wget 설치는 link:../Development-Environments/development-environments.html#\_wget_설치[이곳]을 참조한다.
[listing]
----
c:\> wget https://github.com/boot-clj/boot-bin/releases/download/2.4.2/boot.exe
...
것이다. 처음 실행하는 경우, boot 실행에 필요한 여러 라비브러리들을 함께 다운로드하므로
시간이 좀 걸린다.
